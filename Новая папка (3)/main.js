const MAX_TOKENS = 999999999;


const skinsData = {
    ww: { name: "Обычная акула", image: "https://raw.githubusercontent.com/sinobu1/shark-tapper/main/ww.png" },
    ww2: { name: "Золотая акула", image: "https://raw.githubusercontent.com/sinobu1/shark-tapper/main/ww2.png" },
    ww3: { name: "Леопардовая акула", image: "https://raw.githubusercontent.com/sinobu1/shark-tapper/main/ww3.png" }
};
...
function feedback() {
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            const SHARK_DEFAULT_SKIN_URL = "https://raw.githubusercontent.com/sinobu1/shark-tapper/main/ww.png";
...
function highlightSelectedSkin() {
    ...
    const selected = document.querySelector(`.skin-option[data-skin="${userData.selectedSkin}"]`);
    if (selected) {
...
function init() {
  ...
  const boostBtn = document.querySelector(".floating-boost");

  if (boostBtn && boostModal && closeBoost) {
    boostBtn.addEventListener("click", () => {
      boostModal.classList.add("active");
      if (typeof userData !== "undefined") {
        document.getElementById("boostTokens").textContent = Math.floor(userData.tokens);
      }
    });
    closeBoost.addEventListener("click", () => {
  ...
  if (counter) {
    counter.textContent = Math.floor(userData.tokens);
  } else {
    console.warn("⚠ Element with ID 'profileTokens' not found.");
  }

  const levelEl = document.getElementById("profileLevel");
  if (levelEl) {
    levelEl.textContent = `Level: ${userData.level} | Coins: ${Math.floor(userData.tokens)}`;
  } else {
    console.warn("⚠ Element with ID 'profileLevel' not found.");
  }
}`);
                ...
                popup.style.fontFamily = 'Rubik', "sans-serif"-serif;
  ...
  if (now - lastPassiveIncomeTime >= passiveIncomeInterval) {
    ...
    userData.tokens = Math.min(userData.tokens + income, MAX_TOKENS);
    updateDisplay();
    highlightSelectedSkin();
    ...
    saveToLocalStorage();
            ...
            function saveProgress(retry = 0) {
  fetch(`${DB_URL}/users/${userId}.json`, {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ ...userData, last_update: Date.now() })
  })
    ...
    .catch(err => {
      ...
      saveToLocalStorage();
      if (retry < 3) {
        setTimeout(() => saveProgress(retry + 1), 1000 * (retry + 1));
      } else {
        alert("⚠ Не удалось синхронизировать данные с сервером.");
      }
    });
}
...
function applyBoost() {
  ...
  if (userData.tokens >= cost) {
    ...
    alert("Недостаточно монет для ускорителя.");
  }
}

// Закрытие boostModal по клику вне окна
window.addEventListener("click", (event) => {
...
function setupBoostModal() {
  ...
  if (boostBtn && boostModal && closeBoost) {
    boostBtn.addEventListener("click", () => {
      ...
      if (typeof userData !== "undefined") {
        document.getElementById("boostTokens").textContent = Math.floor(userData.tokens);
        document.getElementById("boostCost").textContent = 100;
      }
    });
    closeBoost.addEventListener("click", () => {
      boostModal.classList.remove("active");
...
window.addEventListener("mousemove", () => lastActiveTime = Date.now());
window.addEventListener("touchstart", () => lastActiveTime = Date.now());

setInterval(() => {
  if (userData.tokens > 0 && Date.now() - lastActiveTime < 60000) {
    saveProgress();
  }
}, AUTO_SAVE_INTERVAL);