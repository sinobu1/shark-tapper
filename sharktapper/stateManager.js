...
class StateManager {
    ...
    setupShopSyncListeners() {
     ...
     //   });
        ...
        this.eventBus.on('shop:itemPurchased', () => this.syncStateFromShop());
     ...
     syncStateFromShop() {
        ...
        const calculatedIncomeMultiplier = incomeMultiplierUpgrade && incomeMultiplierUpgrade.owned > 0
            ? Math.pow(incomeMultiplierUpgrade.effect, incomeMultiplierUpgrade.owned)
            : 1;
        ...
        // Синхронизация энергии и регенерации (пример)
        if (this.state.maxEnergy !== Config.game.maxEnergy) {
            updates.maxEnergy = Config.game.maxEnergy;
            needsUpdate = true;
        }
    
        if (needsUpdate) {
            this.updateState(updates);
            console.log("Synced state from Shop:", updates);
        }
        ...
        // или использовать значения из state.
         if (Config.game.maxEnergy !== calculatedMaxEnergy) {
             ...
             if (this.state.maxEnergy !== calculatedMaxEnergy) {
                updates.maxEnergy = calculatedMaxEnergy;
                needsUpdate = true;
            }// Осторожно с глобальной мутацией
    ...
    handleTap(event) {
        ...
        if (window.Telegram?.WebApp?.HapticFeedback?.impactOccurred) {
           ...
           // Применяем множитель крита и буста
           ...
           if (isCritical) {
            ...
            energy: state.energy - Config.game.energyCostPerTap
        };
         ...
         if (newProgress >= newProgressToNextLevel) {
             ...
             newProgressToNextLevel = Math.max(100, Math.floor(state.progressToNextLevel * 1.5)); // Формула следующего уровня
              ...
              if (newLevel % 5 === 0) {
                  ...
                  this.state.baseTapValue += 1; // Или добавляем в updates ниже
    ...
    if (newState[type] !== undefined) {
        ...
        this.updateState(newState);
        GameState.saveState(); // Синхронизация с GameState
    }
