...
class UIManager {
  ...
  updateUI(state) {
    ...
    // Обновляем изображение акулы (тапалки) по выбранному скину
    const sharkImage = document.querySelector("img.shark");
    const currentSkin = state.shop.skins.current;
    if (sharkImage && config.skins[currentSkin]) {
      sharkImage.src = config.skins[currentSkin].imageUrl;
    }
      ...
      const itemHTML = `
            <div class="shop-item-card ${
              itemConfig.rarity
                ...
                <!-- Skin Preview -->
                <div class="skin-preview" style="background-image: url('${
                  itemConfig.imageUrl
                }');"></div>
        ...
        itemConfig.rarity
                ...
                <div class="card-body">
                    <div class="card-description">+${
                      itemConfig.income
                    } /сек. Уровень: ${itemState.owned}</div>
    ...
    Object.entries(config.lootboxes).forEach(([id, itemConfig]) => {
      ...
      if (sharkImage && config.skins[currentSkin]) {
        ...
        eventBus.on("state:updated", (state) => {
          console.log("Current skin:", state.shop.skins.current);
        });
      }
    });
  }
