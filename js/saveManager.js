...
// Импортируем сам экземпляр leaderboardManager, который создается в main.js
// Убедитесь, что main.js выполняется и создает window.leaderboardManager до того,
// как saveManager попытается его использовать. Порядок импортов в main.js важен.
// !!! ВНИМАНИЕ: Прямой импорт может вызвать проблемы с циклическими зависимостями
// при инициализации. Безопаснее использовать window.leaderboardManager
let leaderboardManager;
setTimeout(() => {
  // Отложенное получение, чтобы main.js успел создать
...
export class SaveManager {
  ...
  saveGame(forceSave = false) {
    ...
    // --- ВЫЗОВ СОХРАНЕНИЯ ЛИДЕРБОРДА ---
    // Используем window.leaderboardManager для избежания проблем импорта
    if (window.leaderboardManager) {
      // Передаем актуальное состояние в лидерборд
      window.leaderboardManager
        .submitScore(stateToSave)
        .catch((err) =>
          console.error("Error submitting score from saveGame:", err)
        );
      // Добавили .catch на случай ошибок при отправке
    } else {
      console.warn(
        "LeaderboardManager not found on window when trying to submit score."
      );
    }
    // --- КОНЕЦ ВЫЗОВА ---
