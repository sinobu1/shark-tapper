//=== SHARK TAPPER — MAIN GAME SCRIPT ===
 ...
 let savedHelpers = {}, savedSkins = {}, savedUpgrades = {};
...
function applySkin(skinId) {
    ...
    if (sharkImg) {
        sharkImg.style.filter = skin.filter;
    }
        
    
    // Пересчитываем статистику, так как множитель дохода мог измениться
...
function recalculateStatsFromItems() {
    ...
    if (savedSkins) {
        for (const id in savedSkins) {
            ...
            if (skinData && savedSkins[id]?.owned) {
                ...
                // Если это текущий выбранный скин, применяем его визуально
                if (id === currentSkinId) {
                    const sharkImg = document.querySelector('.shark');
                    if (sharkImg) {
                        sharkImg.style.filter = skinData.filter;
                    }
                }
            }
        }
 ...
 function initializeDefaultState() {
     ...
     baseTapValue = 1; totalCoins = 0; totalGems = 0; totalOre = 0; totalTaps = 0;
     incomeMultiplier = 1; criticalChance = 0.05; criticalMultiplier = 2; oreChance = 0.03;
     energyRegenRate = 1; energyCostPerTap = 1; currentSkinId = 'basic';
