...
function feedback() {
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            function buyHelper(helperId) {
                ...
                const helper = helpersData[helperId];
                if (userData.tokens >= helper.cost && (userData.helpers[helperId] || 0) < 10) {
...
updateSkinLabels();
                    ...
                    if ((userData.helpers[helperId] || 0) >= 10) {
                        console.log("Максимум помощников этого типа достигнут");
                        alert("Нельзя купить больше 10 этого помощника");
                        return;
                    }
            ...
            function setupEventListeners() {
                ...
                skinsGallery.forEach(img => {
                    img.addEventListener('click', function() {
                        ...
                        if (!userData.unlockedSkins.includes(selectedSkin)) {
                            const cost = selectedSkin === 'skin2' ? 500 : selectedSkin === 'skin3' ? 1000 : 0;
                            if (userData.tokens >= cost) {
                                if (confirm(`Купить скин за ${cost} монет?`)) {
                                    userData.tokens -= cost;
                                    userData.unlockedSkins.push(selectedSkin);
                                    saveProgress();
                                    updateDisplay();
                                } else {
                    if ((userData.helpers[helperId] || 0) >= 10) {
...
updateSkinLabels();
            ...
            function createCoinPopup(event, amount) {
                ...
                popup.style.top = `${Math.max(20, event.clientY - 20)}px`;
...
updateSkinLabels();
    ...
    unlockedSkins: userData.unlockedSkins || ['default']
                    })
                })
            ...
            function initLocalStorage() {
                ...
                if (!isNaN(lastOnline)) {
                    ...
                    const offlineIncome = getPassiveIncome() * secondsOffline;
                    userData.tokens += offlineIncome;
                    console.log(`Оффлайн доход: ${offlineIncome} монет за ${secondsOffline} сек`);
                }
...
userData.unlockedSkins = parsed.unlockedSkins || ['default'];
                        updateDisplay();
    highlightSelectedSkin();
updateSkinLabels();
...
updateSkinLabels();
            ...
            function setSkin(skin, silent = false) {
                ...
                // Проверка: скин должен быть куплен
                const unlocked = userData.unlockedSkins || ['default'];
                if (!unlocked.includes(skin)) {
                    alert("Этот скин ещё не куплен!");
                    return;
                }
...
function updateSkinLabels() {
    ...
    const unlocked = userData.unlockedSkins || ['default'];
    document.querySelectorAll('.skin-price-label').forEach(label => {
        const skin = label.dataset.skin;
        if (unlocked.includes(skin)) {
            label.textContent = '✔ Куплен';
            ...
            label.textContent = `Купить за ${skinPrices[skin]} монет`;
            label.style.color = 'var(--subtext)';
        }
        label.style.fontSize = '0.65rem';
        label.style.fontFamily = 'Rubik, sans-serif';
